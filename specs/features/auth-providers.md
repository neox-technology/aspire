...
- Auth app registrations do **not** implement `IResourceWithWaitSupport` (avoids Aspire blocking custom Auth resources on sibling waits); `WithApiPermission` still records a `WaitAnnotation` + WaitFor relationship for the dashboard graph.
