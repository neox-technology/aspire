...
- **Dashboard status (Entra):** AuthOps resources publish `Waiting` or `Running` + `HealthStatus` (`Healthy` / `Unhealthy`) via `ResourceNotificationService`. Leaf existence is probed with Microsoft Graph (including redirect URI buckets); parents aggregate children with **worst-wins** (Unhealthy > Waiting > Healthy). App registrations are Unhealthy when Graph redirect URIs differ from desired-state. Google resources stay static `Running` (no dashboard probe in this revision). Dashboard commands: **Select tenant** on the Entra provider, **Select or create app registration** + **Provision app registration** on each Auth app. Tenant and ClientId use an empty published default so they stay out of Aspire’s startup unresolved-parameters modal (secrets may still appear there); Choice prompts live on the Select commands (and on pipeline `EnsureReadyAsync`). Dashboard refresh starts on Aspire `BeforeStartEvent` (not `AfterResourcesCreatedEvent`) so `WithAuth` WaitFor cannot deadlock status publishing.

## User scenarios

- AppHost registers `AddAuthProvider("auth-provider-entra").Entra(...)`, defines one or more apps with `AddAppRegistration(name, displayName)`, binds consumers with Entra `WithAuth(app)`; local interactive `aspire do` prompts unresolved parameters via pipeline `EnsureReadyAsync` (tenant Choice of accessible tenants; ClientId FluentSelect of Create + app registrations in that tenant + Other, with a follow-up Text prompt for a custom GUID); AppHost **run mode** resolves tenant / ClientId via dashboard Select commands (not the startup unresolved modal); CI supplies `Parameters__*` / Azure credential with `--non-interactive`.
...
- Unit tests under `tests/auth-providers/tests/` use fakes (no live Graph / no `az`).
- **Dashboard (Entra run mode):** scopes/app roles/api permissions start `Waiting`, then `Running` Healthy/Unhealthy from Graph presence (and stay `Waiting` while the parent app registration is not Running+Healthy). In-model `AuthApiPermission` stays `Waiting` while the exposer’s matching scope/role is not Healthy. App registrations wait until ClientId is set, then probe Graph existence and aggregate child exposition + permission health; consumer apps stay `Waiting` while an in-model exposer is not Healthy. Providers wait until TenantId is set, then aggregate apps. `auth-ops` aggregates Entra providers. Typical run-mode order: **Select tenant** (provider) → **Select or create app registration** (Auth app) → **Provision app registration** → **Create client secret**. Tenant / ClientId Choice are **not** in the startup unresolved-parameters modal (empty published defaults); client-secret parameters may still appear there. **Select tenant** is enabled only while TenantId is unset. **Select or create app registration** is enabled when TenantId is set, ClientId is unset/create-sentinel, and every Auth-app `WaitAnnotation` target is Healthy (no WaitFor Auth deps → tenant + ClientId unset only). **Provision app registration** runs `PlanAsync`, prompts confirmation with mode + bulleted planned Graph actions, then on confirm runs `ProvisionAsync` and refreshes status (disabled while provisioning / tenant unset; disabled while Waiting unless ClientId is the explicit create sentinel; Cancel skips Graph writes; unavailable `IInteractionService` fails the command).
...
- Aspire dashboard `WithCommand` on `EntraAuthOpsResource` (**Select tenant**) and on `EntraAuthAppRegistrationResource` (**Select or create app registration**, **Provision app registration**).
...
- [x] `EntraAuthProviderOptions.TenantId` is `IResourceBuilder<ParameterResource>?`; auto-creates `{name}-tenant-id` when null with an **empty published default** so it is not in Aspire’s startup unresolved-parameters modal; dashboard **Select tenant** (and pipeline `EnsureReadyAsync`) own the Choice prompt.
- [x] Per Auth app, `prereq-{app}-auth` **DependsOn** `prereq-providers-auth`; ClientId parameter uses an **empty published default** (same modal exclusion); Choice (FluentSelect: create sentinel + Graph apps + Other; Other opens a follow-up Text GUID prompt — no `AllowCustomChoice` Combobox) is owned by dashboard **Select or create app registration** and by pipeline `EnsureReadyAsync`; `DisplayName` is a required `AddAppRegistration` argument (not a ParameterResource).
- [x] `prereq-{app}-auth` ClientId Choice prefetches Graph app registrations in the selected tenant via `EnsureReadyAsync` (pipeline); run-mode Select command enumerates Graph apps for the resolved tenant (no model-time `DynamicLoading` / no cross-parameter `DependsOnInputs`).
- [x] Run mode: tenant and ClientId parameters are absent from the startup unresolved-parameters modal; client-secret parameters may still appear there.
- [x] `EntraAuthOpsResource` exposes dashboard command `select-tenant` (“Select tenant”): Choice of ARM tenants (+ custom GUID); persists via `AuthParameterValue`; enabled only while TenantId is unset.
- [x] `EntraAuthAppRegistrationResource` exposes dashboard command `select-app-registration` (“Select or create app registration”): FluentSelect of Create + Graph apps + Other (Other → second Text prompt for Client ID GUID); persists real ClientId (create sentinel is set in-memory only, not written as deployment-state ClientId); empty/unusable Choice returns Failure (not silent Cancel); **after Create is confirmed, immediately chains into the same provision confirmation flow as `provision-auth`** (Cancel on provision leaves the create sentinel set so Provision remains available); enabled when TenantId is resolved, ClientId is unset or create-sentinel, every Auth-app `WaitAnnotation` target is Healthy (or there are no such deps), and provision is not running.
...
- [x] `EntraAuthAppRegistrationResource` exposes dashboard command `provision-auth` (“Provision app registration”) that runs `PlanAsync`, prompts confirmation with mode + bulleted planned Graph actions via `IInteractionService`, then on confirm runs `ProvisionAsync` and refreshes status; Cancel returns `CommandResults.Canceled` with no Graph writes; unavailable interaction service fails the command; enabled when TenantId is resolved, the command is not already running, and either the app status is not Waiting **or** ClientId is the explicit create sentinel (`__create__`).
- [x] Unit tests cover worst-wins aggregation, Graph probe existence mapping (fakes), command annotation presence / UpdateState enable rules for Select tenant / Select app / Provision, provision confirmation message formatting, empty published defaults for tenant/ClientId, and Entra initial `Waiting` states.
