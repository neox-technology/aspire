...
- **Dashboard (Entra run mode):** scopes/app roles/api permissions start `Waiting`, then `Running` Healthy/Unhealthy from Graph presence (and stay `Waiting` while the parent app registration is not Running+Healthy). App registrations wait until ClientId is set, then probe Graph existence and aggregate child exposition + permission health. Providers wait until TenantId is set, then aggregate apps. `auth-ops` aggregates Entra providers. A **Provision app registration** command on each Entra app registration re-runs plan + provision and refreshes status.
...
- [x] In-model `WithApiPermission`: consumer Auth app dashboard status (and health-check cache) worst-wins with each distinct exposer Auth app’s own status (two-pass refresh); consumer is not Healthy while an exposer is Waiting/Unhealthy; well-known and self-owned do not gate; Auth→Auth `WaitAnnotation` remains non-orchestrated (no `IResourceWithWaitSupport`).
- [x] Unit tests cover consumer Waiting/Unhealthy when exposer is Waiting/Unhealthy, and Healthy when exposer + permission are Healthy.
...
| `AuthApiPermission` | Default; parent Graph app missing | Matching `requiredResourceAccess` present on consumer Graph app | Parent exists but entry missing / Graph error |
