...
- [x] Per Auth app, `prereq-{app}-auth` **DependsOn** `prereq-providers-auth`; ClientId parameter uses an **empty published default** (same modal exclusion); Choice (create sentinel + `AllowCustomChoice` GUID) is owned by dashboard **Select or create app registration** and by pipeline `EnsureReadyAsync`; `DisplayName` is a required `AddAppRegistration` argument (not a ParameterResource).
...
- [x] `EntraAuthAppRegistrationResource` exposes dashboard command `select-app-registration` (“Select or create app registration”): Choice of Create + Graph apps + custom GUID; persists real ClientId (create sentinel is set in-memory only, not written as deployment-state ClientId); enabled when TenantId is resolved, ClientId is unset or create-sentinel, every Auth-app `WaitAnnotation` target is Healthy (or there are no such deps), and provision is not running.
