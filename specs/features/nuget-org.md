...
- [x] Spec documents PR → `main` CI (pack `*-ci`, no push) and merge → publish to nuget.org via Trusted Publishing.
...
- [x] `.github/workflows/publish-nuget.yml` uses `NuGet/login@v1` + `id-token: write` and pushes Shipping packages to `https://api.nuget.org/v3/index.json`.
