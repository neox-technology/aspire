...
- Bootstrap: deploy with empty cert → DomainOps DNS + hostname + managed cert + bind → redeploy with `Parameters__certificateName` (GH variable automation is out of scope V1).
...
- `Parameters__customDomain`, `Parameters__certificateName` (empty allowed on bootstrap deploy)
