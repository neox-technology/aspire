...
GitFlow **branch convention** for this repo: `feature/*` from `develop`; `release/*` / `hotfix/*` to `main`; `main` holds shipped releases. Manual GitHub Actions cut a release from `develop`, publish a private daily feed from `release/*`, and finalize (squash-merge into `main` and `develop`, tag `v*`, publish nuget.org). NuGet identity lives in Arcade [`eng/Versions.props`](../../eng/Versions.props). Pack/publish details: [`nuget-org`](nuget-org.md). No Aspire deploy workflow.
...
4. **Linear history** — finalize squash-merges the release branch into `main` and `develop` (no release PRs).
5. **Git automation identity** — branch/tag/push steps use org App **`neox-gitflow`** (`NEOX_GITFLOW_APP_ID` / `NEOX_GITFLOW_APP_PRIVATE_KEY`), not a long-lived PAT.
