...
| **prereq-domain-{provider}** | Provider-specific DomainOps prereq (`cloudflare` \| `ovh`) that `docker pull`s `octodns/{provider}`; depends on `prereq-domain`. One step per provider slug, shared across resources of that type. |
