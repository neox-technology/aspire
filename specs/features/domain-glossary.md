...
| **Auth app** | Logical app registration under an Auth provider (`AddAppRegistration` / `AuthAppResource`) with a required display name; redirect URI desired-state via `WithRedirectUri` / `WithLocalhostRedirectUri`; Entra supported accounts via `WithSupportedAccounts` (Graph `signInAudience`); API exposition via `WithApiExposition` / `WithAppRoleExposition`; consume exposed scopes/roles via `WithApiPermission`; further registration settings via future `WithXxx` methods. |
