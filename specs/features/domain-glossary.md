...
| **Auth provider** | Non-container Aspire resource (`AuthProviderResource`) that selects an identity **provider** (v1: Entra), holds management/workload parameter bindings, and owns AuthOps pipeline gates (`prereq-auth-entra`, `deploy-auth`) for that provider. |
...
| **prereq-auth-entra** | Entra-specific AuthOps prereq hosted on the Entra provider resource: tenant parameter ready and Graph credential path usable. |
