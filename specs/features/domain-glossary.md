...
| **AuthOps** | Hosting helpers split as `Neox.Aspire.Hosting.Auth.Abstractions` (common model, `WithAuth`) + provider packages (`Neox.Aspire.Hosting.Auth.EntraId`, `Neox.Aspire.Hosting.Auth.Google`) that provision identity-provider **app registrations** / OAuth clients and inject workload credentials via generic `AUTH_*` environment variables through `aspire do` / `aspire deploy` pipeline steps. |
...
| **Auth app** | Logical app registration under an Auth provider (`AddAppRegistration` / `AuthAppResource`) with a required display name; redirect URI desired-state via Abstractions `WithRedirectUri` / `WithLocalhostRedirectUri` (flat list); Entra adds Graph platform buckets via typed overloads (`AuthApplicationType`) plus `WithSupportedAccounts` / API exposition / `WithApiPermission`. |
...
| **API permission** | Consumer Auth app dependency on an exposed scope or app role (`WithApiPermission` against an in-model `ApiExposition`) **or** a **well-known API permission** (first-party Microsoft Graph); Graph `requiredResourceAccess` (type Scope or Role). |
| **Well-known API permission** | First-party Microsoft Graph delegated scope or application role bound via `WithApiPermission(WellKnownApiPermission)` using source-generated `MicrosoftGraph.Delegated` / `MicrosoftGraph.Application` constants; `ResourceAppId` is the Microsoft Graph app id (`00000003-0000-0000-c000-000000000000`). Does not create an Aspire child resource or exposer `DependsOn`. |
