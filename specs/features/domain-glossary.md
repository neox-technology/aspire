...
| **Auth app** | Logical app registration under an Auth provider (`AddAppRegistration` / `AuthAppResource`) with a required display name; redirect URI desired-state via `WithRedirectUri` / `WithLocalhostRedirectUri`; Entra supported accounts via `WithSupportedAccounts` (Graph `signInAudience`); further registration settings via future `WithXxx` methods. |
...
| **provision-{app}-auth** | Applies the Auth app plan via Microsoft Graph (create with DisplayName + redirect URIs + `signInAudience`, patch those, or noop bind), sets workload ClientId/TenantId `ParameterResource`s; **DependsOn** `plan-{app}-auth`. |
